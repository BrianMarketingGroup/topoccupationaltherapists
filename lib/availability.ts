...
import { getCache, setCache } from "./availabilityCache";
...
/**
 * Currently-claimed Featured cities. Cached for 60s (see lib/availabilityCache.ts)
 * since this backs the apply form's availability check; pass `fresh: true` to
 * bypass the cache, which the apply API route does right before accepting a
 * submission so a slot claimed moments ago can't be sold again.
 */
export async function getTakenCities(opts?: { fresh?: boolean }): Promise<{ state: string; city: string }[]> {
  if (!opts?.fresh) {
...
export function featuredCityKey(city: string, state: string): string {
